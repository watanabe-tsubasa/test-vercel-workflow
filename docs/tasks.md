...
- [ ] デプロイ前の `npm run lint` / `npm run format` / 簡易動作確認

## step.6 追加機能
- [ ] アクセス時、作成中の日記が存在する場合（DB上にCOMPLETE以外のstatusが存在する場合）、`作成中の日記が存在します`のモーダルを表示し、ボタンクリックでそのページに飛ぶように変更
- [ ] 作成したドラフトの日記文章をSSEでフロントにストリーミングし、フロント側で受け取り反映
- [ ] 日記表示ページ`/diary/[id]`で固定画像ではなく、Neonに保存されているimagekit urlを取得するように変更
- [ ] 初稿ステータスが「編集待ち」で文章を表示できるようにし、修正送信で DRAWING→COMPLETED へ進む
- [ ] ImageKit アップロード失敗時にプレースホルダーで完了し、`hasImage` を正しく設定する（現在 ??? 画像が保存される問題の解消）
- [ ] モバイル時に /diary や /creation のサイドバーを隠し、出し入れ or モーダル化する
- [ ] 環境変数不足時でもモックドラフト/プレースホルダーでフローが継続することを確認・担保する
